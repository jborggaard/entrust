...
%-------------------------------------------------------------------------------
  ...
  if strcmp( options.method,'secant' )
    if ( isfield(options,'initial_hessian') )
      ...
      H = max(abs(f),options.scale_f)*eye(n_desvar); 
    end 
  end
  ...
  while ( converged ~= 1 )
    ...
    if strcmp( options.globalization,'none' )
      ...
      if strcmp( options.method,'newton' )
        ...
        g_plus = g_plus(:);
        if ( strcmp(options.goal(1:3), 'max') )
          f_plus =-f_plus; 
          g_plus =-g_plus; 
          H_plus =-H_plus;
        end
    ...
    if strcmp( options.globalization,'trust_region' )
      if ( isfield(options,'tr_radius') ~= 1 )
        options.tr_radius = min( abs(g'*g/(g'*H*g)),options.max_step); 
      end
...
%------------------------------------------------------------------------
  ...
  if ( isfield(options,'scale_f') ~= 1 )
    ...
    options.scale_f = abs(options.scale_f); 
  end

  %  Set Bound Constraints
  if ( isfield(options,'x_lower') ~= 1 )
    options.x_lower =-realmax*ones(n_desvar,1); 
  else 
    if ( ~strcmp( options.globalization, 'trust_region' ) )
      error(strcat(' Bound constraints are only implemented with',...
                   ' trust_region globalization option') )
    end
  end
      
  if ( isfield(options,'x_upper') ~= 1 )
    options.x_upper = realmax*ones(n_desvar,1);
  else 
    if ( ~strcmp( options.globalization, 'trust_region' ) )
      error(strcat(' Bound constraints are only implemented with',...
                   ' trust_region globalization option') )
